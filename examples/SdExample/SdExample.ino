...
void setup()
{
    ...
    logger.registerSerial(MAIN, DEBUG, "count", Serial, FLAG_NO_SERVICE | FLAG_NO_LEVEL); // On serial, we don't want to show the service name and log level
    logger.registerSd(MAIN, DEBUG, "main");
    logger.registerSd(SUB_COUNTER, ERROR, "subcount"); // We only want to log errors
    logger.registerSd(HEX_LOG, DEBUG, "hex");
