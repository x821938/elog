...
public:
    ...
    void write(const LogLineEntry logLineEntry, SdFileSetting& setting);
...
private:
    ...
    SdFileSetting* fileSettings; // Array of file settings
    ...
    void ensureFileSize(SdFileSetting& setting);
    ...
    void getSettingFullFileName(char* output, SdFileSetting setting);
    void timestampFile(SdFileSetting& setting);
    uint32_t convertToEpoch(uint16_t pdate, uint16_t ptime);

    void createLogFileIfClosed(SdFileSetting& setting);
    void allFilesClose();
    void allFilesSync();
};
